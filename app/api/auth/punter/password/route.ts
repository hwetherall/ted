...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Enter your nickname." }, { status: 400 });
  const result = await verifyNicknameLogin(parsed.data.punterId, parsed.data.nickname, true);
